...
def _normalize_http_headers(headers):
    ...
    try:
        ...
        return {}
    return {str(key): str(value) for key, value in items if key and value is not None}
...
def _direct_http_formats(info, *, audio_only=False):
    ...
    candidates = [fmt for fmt in formats if isinstance(fmt, dict) and is_direct_http(fmt)]
    if audio_only:
...
def extract_media(url, fmt='best', audio_only=False, single_video=True):
    ...
    return {
        ...
        'http_headers': _normalize_http_headers(selected.get('http_headers') or info.get('http_headers')),
    }
